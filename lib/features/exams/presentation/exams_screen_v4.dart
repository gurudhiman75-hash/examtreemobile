  ...
  Future<void> _refresh() async {
    ...
    Future<void> settle(Future<Object?> request) async {
      try {
        ...
        // The catalogue and saved attempts recover independently.
      }
    }
          ...
          Expanded(
            child: SizedBox(
              ...
              child: ListView(
                ...
                scrollDirection: Axis.horizontal,
                children: [
                  for (final option in ExamAccessFilter.values) ...[
                    ChoiceChip(
                      label: Text(option.label),
                      selected: access == option,
                      onSelected: (_) => onAccessChanged(option),
                    ),
                    if (option != ExamAccessFilter.values.last)
                      const SizedBox(width: AppSpacing.xs),
                  ],
                ],
              ),
...
class _ResumeRail extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final textScale = MediaQuery.textScalerOf(context).scale(1);
    final itemWidth = (screenWidth * 0.77).clamp(260.0, 320.0).toDouble();
    final railHeight = textScale > 1.5 ? 292.0 : 174.0;
...
class _AccessBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final background =
        paid ? scheme.tertiaryContainer : scheme.secondaryContainer;
    final foreground =
        paid ? scheme.onTertiaryContainer : scheme.onSecondaryContainer;
    return Container(
      padding: const EdgeInsets.symmetric(
        horizontal: AppSpacing.sm,
        vertical: AppSpacing.xs,
