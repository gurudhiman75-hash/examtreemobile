...
class LearnScreen extends ConsumerWidget {
  ...
  Future<void> _refresh(WidgetRef ref) async {
    ...
    try {
      ...
      // Each Learn module owns its own retry state.
    }
    try {
      await ref.read(availableExamsProvider.future);
    } catch (_) {
      // Published resources can still render if tests are unavailable.
    }
  }
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Text(
              'Some free learning content could not be refreshed. The items shown above are still available.',
              style: theme.textTheme.bodySmall?.copyWith(
                color: theme.colorScheme.onSurfaceVariant,
                height: 1.4,
              ),
